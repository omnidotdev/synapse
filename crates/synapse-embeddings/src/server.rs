...
impl Server {
    ...
    pub async fn embed(
        ...
        #[cfg(feature = "billing")]
        {
            ...
            self.record_usage(context, pname, &request.model, response.usage.prompt_tokens, &request_id);
            self.deduct_credits(context, pname, &request.model, response.usage.prompt_tokens, &request_id)
                .await;
        }
    ...
    #[cfg(feature = "billing")]
    fn record_usage(&self, context: &RequestContext, provider_name: &str, model_id: &str, prompt_tokens: u32, request_id: &str) {
        ...
        recorder.record(synapse_billing::UsageEvent {
            ...
            input_tokens: if is_byok { 0 } else { prompt_tokens },
            output_tokens: 0,
    ...
    #[cfg(feature = "billing")]
    async fn deduct_credits(&self, context: &RequestContext, provider_name: &str, model_id: &str, prompt_tokens: u32, request_id: &str) {
        let Some(ref client) = self.billing_client else {
            return;
        };
