...
pub async fn csrf_middleware(header_name: String, request: Request, next: Next) -> Response {
    ...
    if let Ok(name) = header {
        ...
        tracing::error!(header = %header_name, "invalid CSRF header name in configuration");
        (StatusCode::INTERNAL_SERVER_ERROR, "CSRF misconfiguration").into_response()
    }
}
