...
impl Server {
    ...
    pub async fn transcribe(
        ...
        let billing_meta = {
            ...
            let audio_bytes: u32 = request.audio.len().try_into().unwrap_or(u32::MAX);
        ...
        #[cfg(feature = "billing")]
        {
            ...
            self.record_usage(context, pname, model_id, audio_bytes, &request_id);
            self.deduct_credits(context, pname, model_id, audio_bytes, &request_id).await;
        }
    ...
    #[cfg(feature = "billing")]
    fn record_usage(&self, context: &RequestContext, provider_name: &str, model_id: &str, audio_bytes: u32, request_id: &str) {
        ...
        recorder.record(synapse_billing::UsageEvent {
            ...
            input_tokens: if is_byok { 0 } else { audio_bytes },
            output_tokens: 0,
    ...
    #[cfg(feature = "billing")]
    async fn deduct_credits(&self, context: &RequestContext, provider_name: &str, model_id: &str, audio_bytes: u32, request_id: &str) {
        let Some(ref client) = self.billing_client else {
            return;
        };
